...
public class TitlesControllerTest {
    ...
    private static Logger logger = LoggerFactory.getLogger(TitlesControllerTest.class);
    ...
    public void list() throws Exception {
        ...
        RequestBuilder requestBuilder = MockMvcRequestBuilders.post("/api/TitlesTable/list")
...
//        JSONAssert.assertEquals(expected,mvcResult.getResponse().getContentAsString(),false);
    ...
    public void create() throws Exception {
        titlesDAO title = new titlesDAO();

        title.setTitle("test_title");
        String content = JSONObject.toJSONString(title);

        RequestBuilder requestBuilder = MockMvcRequestBuilders.post("/api/TitlesTable/create")
                .contentType(MediaType.APPLICATION_JSON_UTF8) //需要指定 只对post请求有效
                .content(content);
        MvcResult mvcResult = mockMvc.perform((requestBuilder)).andExpect(status().isOk()).andReturn();
        String stringResult = mvcResult.getResponse().getContentAsString();
...
//        JSONAssert.assertEquals(expected,mvcResult.getResponse().getContentAsString(),false);
    ...
    public void update() throws Exception {
        ...
        title.setId(7);
        title.setTitle("test_title_changed");
        String content = JSONObject.toJSONString(title);

        RequestBuilder requestBuilder = MockMvcRequestBuilders.post("/api/TitlesTable/update")
                .contentType(MediaType.APPLICATION_JSON_UTF8) //需要指定 只对post请求有效
                .content(content);
        MvcResult mvcResult = mockMvc.perform((requestBuilder)).andExpect(status().isOk()).andReturn();
        String stringResult = mvcResult.getResponse().getContentAsString();
...
//        JSONAssert.assertEquals(expected,mvcResult.getResponse().getContentAsString(),false);
    ...
    public void remove() throws Exception {
        ...
        RequestBuilder requestBuilder = MockMvcRequestBuilders.post("/api/TitlesTable/remove")
                .contentType(MediaType.APPLICATION_JSON_UTF8) //需要指定 只对post请求有效
                .content(content);
        MvcResult mvcResult = mockMvc.perform((requestBuilder)).andExpect(status().isOk()).andReturn();
        String stringResult = mvcResult.getResponse().getContentAsString();
